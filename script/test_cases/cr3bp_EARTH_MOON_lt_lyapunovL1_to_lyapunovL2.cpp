...
SolverParameters get_SolverParameters_cr3bp_EARTH_MOON_lt_lyapunovL1_to_lyapunovL2(
	...
	vectordb homotopy_sequence{0, 0.5, 0.85, 0.99, 0.99};
	vectordb huber_loss_coefficient_sequence{1e-2, 2e-3, 1e-3, 1e-3, 5e-4};
	double DDP_tol = 1e-4;
	double AUL_tol = 1e-6;
	double PN_tol = 1e-12;
...
void cr3bp_EARTH_MOON_lt_lyapunovL1_to_lyapunovL2(int argc, char** argv) {
	...
	if (monte_carlo_validaiton) {
		...
		// Print datasets
		if (save_results) {
			string file_name = "./data/sample_trajectory/cr3bp_EARTH_MOON_lt_lyapunovL1_to_lyapunovL2";
			string system_name = "CR3BP EARTH-MOON CARTESIAN LT";
